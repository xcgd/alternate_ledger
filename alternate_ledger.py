...
class alternate_ledger_move(osv.osv):
    ...
    def alternate_ledger_move_prepare(self, cr, uid, journal_id, date=False, ref='', company_id=False, context=None):
        '''
    ...
    def name_get(self, cursor, user, ids, context=None):
        ...
        if not ids:
            return []
        res = []
        data_move = self.pool.get('alternate_ledger.move').browse(cursor, user, ids, context=context)
    ...
    def _search_amount(self, cr, uid, obj, name, args, context):
        ...
        for cond in args:
            ...
            cr.execute("select move_id from alternate_ledger_move_line group by move_id having sum(debit) %s %%s" % (cond[1]),(amount,))
    ...
    def post(self, cr, uid, ids, context=None):
        ...
        cr.execute('UPDATE alternate_ledger_move '\
                   'SET state=%s '\
                   'WHERE id IN %s',
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        c = context.copy()
        c['novalidate'] = True
        result = super(alternate_ledger_move, self).write(cr, uid, ids, vals, c)
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if 'line_id' in vals:
            ...
            result = super(alternate_ledger_move, self).create(cr, uid, vals, c)
            self.validate(cr, uid, [result], context)
        else:
            result = super(alternate_ledger_move, self).create(cr, uid, vals, context)
        return result
    ...
    def copy(self, cr, uid, id, default=None, context=None):
        ...
        return super(alternate_ledger_move, self).copy(cr, uid, id, default, context)
    ...
    def unlink(self, cr, uid, ids, context=None, check=True):
        ...
        result = super(alternate_ledger_move, self).unlink(cr, uid, toremove, context)
        return result
    ...
    def _centralise(self, cr, uid, move, mode, context=None):
        ...
        cr.execute('select id from alternate_ledger_move_line where move_id=%s and centralisation=%s limit 1', (move.id, mode))
        res = cr.fetchone()
        if res:
            line_id = res[0]
        else:
            context.update({'journal_id': move.journal_id.id, 'period_id': move.period_id.id})
            line_id = self.pool.get('alternate_ledger.move.line').create(cr, uid, {
        ...
        cr.execute('select id from alternate_ledger_move_line where move_id=%s and centralisation=%s limit 1', (move.id, mode2))
        res = cr.fetchone()
        if res:
            line_id2 = res[0]
        else:
        ...
        cr.execute('SELECT SUM(%s) FROM alternate_ledger_move_line WHERE move_id=%%s AND id!=%%s' % (mode,), (move.id, line_id2))
        result = cr.fetchone()[0] or 0.0
        cr.execute('update alternate_ledger_move_line set '+mode2+'=%s where id=%s', (result, line_id))

        #adjust also the amount in currency if needed
        cr.execute("select currency_id, sum(amount_currency) as amount_currency from alternate_ledger_move_line where move_id = %s and currency_id is not null group by currency_id", (move.id,))
        for row in cr.dictfetchall():
            ...
            if not currency_obj.is_zero(cr, uid, currency_id, row['amount_currency']):
                ...
                cr.execute('select id from alternate_ledger_move_line where move_id=%s and centralisation=\'currency\' and currency_id = %slimit 1', (move.id, row['currency_id']))
                res = cr.fetchone()
                if res:
                    cr.execute('update alternate_ledger_move_line set amount_currency=%s , account_id=%s where id=%s', (amount_currency, account_id, res[0]))
                else:
                    context.update({'journal_id': move.journal_id.id, 'period_id': move.period_id.id})
                    line_id = self.pool.get('alternate_ledger.move.line').create(cr, uid, {
...
class alternate_ledger_move_reconcile(osv.osv):
    ...
    _columns = {
        ...
        'line_id': fields.one2many('alternate_ledger.move.line', 'reconcile_id', 'Entry Lines'),
        'line_partial_ids': fields.one2many('alternate_ledger.move.line', 'reconcile_partial_id', 'Partial Entry lines'),
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        return super(alternate_ledger_move_reconcile, self).unlink(cr, uid, ids, context=context)
    ...
    def reconcile_partial_check(self, cr, uid, ids, type='auto', context=None):
        ...
        if not total:
            self.pool.get('alternate_ledger.move.line').write(cr, uid,
