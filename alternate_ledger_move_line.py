...
class alternate_ledger_move_line(osv.osv):
    ...
    def _amount_residual(self, cr, uid, ids, field_names, args, context=None):
        """
           This function returns the residual amount on a receivable or payable alternate_ledger.move.line.
    ...
    def _prepare_analytic_line(self, cr, uid, obj_line, context=None):
        ...
        :param obj_line: browse record of the alternate_ledger.move.line that triggered the analytic line creation
        """
    ...
    def _default_get_move_form_hook(self, cursor, user, data):
        '''Called in the end of default_get method for manual entry in alternate_legder_move form'''
    ...
    def _balance(self, cr, uid, ids, name, arg, context=None):
        ...
        sql = """SELECT l2.id, SUM(l1.debit-l1.credit)
                    FROM alternate_ledger_move_line l1, alternate_ledger_move_line l2
    ...
    def _invoice(self, cursor, user, ids, name, arg, context=None):
        ...
        cursor.execute('SELECT l.id, i.id ' \
                        'FROM alternate_ledger_move_line l, account_invoice i ' \
    ...
    def _get_move_lines(self, cr, uid, ids, context=None):
        result = []
        for move in self.pool.get('alternate_ledger.move').browse(cr, uid, ids, context=context):
            print move
    ...
    _columns = {
        ...
        'narration': fields.related(
            'move_id',
            'narration',
            type='text',
            relation='alternate_ledger.move',
        ...
        'amount_currency': fields.float(
            'Amount Currency',
            help="""The amount expressed in an optional other
        ...
        'amount_residual_currency': fields.function(
            ...
            multi="residual",
            help="""The residual amount on a receivable or payable of a
        ...
        'amount_residual': fields.function(
            ...
            multi="residual",
            help="""The residual amount on a receivable or payable of
        ...
        'blocked': fields.boolean(
            'No Follow-up',
            help="""You can check this box to mark this journal
        ...
        'tax_code_id': fields.many2one(
            'account.tax.code',
            'Tax Account',
            help="""The Account can either be a base tax
        ...
        'tax_amount': fields.float(
            ...
            digits_compute=dp.get_precision('Account'),
            select=True,
            help="""If the Tax account is a tax code account, this field
    ...
    def _get_date(self, cr, uid, context=None):
        ...
        if context.get('journal_id') and context.get('period_id'):
            cr.execute('SELECT date FROM alternate_ledger_move_line ' \
    ...
    _defaults = {
        ...
        'company_id': lambda self, cr, uid, c: self.pool.get('res.company')._company_default_get(cr, uid, 'alternate_ledger.move.line', context=c)
    }
    ...
    def _auto_init(self, cr, context=None):
        super(alternate_ledger_move_line, self)._auto_init(cr, context=context)
        cr.execute('SELECT indexname FROM pg_indexes WHERE indexname = \'account_move_line_journal_id_period_id_index\'')
        if not cr.fetchone():
            cr.execute('CREATE INDEX account_move_line_journal_id_period_id_index ON alternate_ledger_move_line (journal_id, period_id)')
    ...
    def search(self, cr, uid, args, offset=0, limit=None, order=None, context=None, count=False):
        ...
        return super(alternate_ledger_move_line, self).search(cr, uid, args, offset, limit, order, context, count)

    def list_partners_to_reconcile(self, cr, uid, context=None):
        cr.execute(
             """SELECT partner_id FROM (
                SELECT l.partner_id, p.last_reconciliation_date, SUM(l.debit) AS debit, SUM(l.credit) AS credit, MAX(l.create_date) AS max_date
                FROM alternate_ledger_move_line l
        ...
        if res:
            if res[1] != 'draft':
                raise osv.except_osv(_('User Error!'),
                       _('The alternate ledger move (%s) for centralisation ' \
    ...
    def _remove_move_reconcile(self, cr, uid, move_ids=None, opening_reconciliation=False, context=None):
        ...
        obj_move_rec = self.pool.get('alternate_ledger.move.reconcile')
        unlink_ids = []
        if not move_ids:
            return True
    ...
    def unlink(self, cr, uid, ids, context=None, check=True):
        ...
        for line in self.browse(cr, uid, ids, context=context):
            ...
            result = super(alternate_ledger_move_line, self).unlink(cr, uid, [line.id], context=context)
    ...
    def write_(self, cr, uid, ids, vals, context=None, check=True, update_check=True):
        ...
        result = super(alternate_ledger_move_line, self).write(cr, uid, ids, vals, context)
        if check:
            done = []
    ...
    def create_(self, cr, uid, vals, context=None, check=True):
        ...
        if vals.get('move_id', False):
            move = self.pool.get('alternate_ledger.move').browse(cr, uid, vals['move_id'], context=context)
            if move.company_id:
        ...
        result = super(alternate_ledger_move_line, self).create(cr, uid, vals, context=context)
        # CREATE Taxes
        if vals.get('account_tax_id', False):
